...
|---|---|---|---|---|
| 1 | AI Health Assistant | ✅ | OpenAI API | Built, working |
...
| 4a | Medicine — OCR prescriptions | ✅ | GPT-4o vision (same pattern as nutrition photos) | Built. Extracted medicines populate the reminder form as suggestions to confirm, never auto-added - handwriting misreads are a real safety issue, not just a UX detail |
...
| 5 | Symptom Checker | ✅ | OpenAI | Built |
...
| 7 | AI Nutrition (photo) | ⚠️→✅ | Dedicated food-recognition APIs (LogMeal, Spike, FatSecret) are commercial only. Built a free workaround instead: GPT-4o vision + your existing key | Built (`analyze_meal_photo`). Accuracy is approximate the same way every photo-nutrition tool's is — surface `confidence`/`note` in the UI |
...
| 15 | Voice Assistant | ✅ | OpenAI Whisper (speech-to-text, multilingual) + on-device TTS (`expo-speech`, free) or OpenAI TTS | Real and buildable. Regional Indian language quality varies by language — test with your actual target languages early, don't assume uniform quality |
